  ...
  : arm_initialized (false)
{
  ...
  fingers_server_				  = new FingersServer( nh, arm_name_ + "_fingers/finger_positions", boost::bind(&JacoArmTrajectoryController::execute_fingers, this, _1), false);
  ...
  GetDevices(devices_list, result);
  ...
  for (int device_i = 0; device_i < MAX_KINOVA_DEVICE/*devices_list.size()*/; device_i++)
  {
      // If no device is specified, just use the first available device
      if ((serial_number_ == "")
          || (std::strcmp(serial_number_.c_str(), devices_list[device_i].SerialNumber) == 0))
      {
          //result = SetActiveDevice(devices_list[device_i]);
          if (result != NO_ERROR_KINOVA)
          {
  ...
  if (!found_arm)
  {
      ROS_ERROR("Could not find the specified arm (serial: %s) among the %d attached devices",
                serial_number_.c_str(), MAX_KINOVA_DEVICE/*static_cast<int>(devices_list.size())*/);
  ...
  cartesian_force_pub_ = nh.advertise<geometry_msgs::WrenchStamped>(arm_name_ + "_arm/cartesian_force", 1);
  joint_cartesian_pub_ = nh.advertise<wpi_jaco_msgs::JointPosiCartForce>(arm_name_ + "_arm/joint_cartesian_states", 1);
  ...
  cartesianForceCmdSubscriber = nh.subscribe(arm_name_+"_arm/cartesian_force_cmd", 1, &JacoArmTrajectoryController::cartesianForceCmdCallback,
                                        this);
  angularTorqueCmdSubscriber = nh.subscribe(arm_name_+"_arm/angular_torque_cmd", 1, &JacoArmTrajectoryController::angularTorqueCmdCallback,
                                      this);
  ...
  angularEffortServer = nh.advertiseService(arm_name_+"_arm/get_angular_effort", &JacoArmTrajectoryController::getAngularEffort, this);
  ...
  startForceControlServer = nh.advertiseService(arm_name_+"_arm/start_force_control", &JacoArmTrajectoryController::startForceControlCallback, this);
  stopForceControlServer = nh.advertiseService(arm_name_+"_arm/stop_force_control", &JacoArmTrajectoryController::stopForceControlCallback, this);
  ...
  cartesian_force_timer_ = nh.createTimer(ros::Duration(0.0333),
                                          boost::bind(&JacoArmTrajectoryController::update_cartesian_force,this));
  joint_cartesian_timer_ = nh.createTimer(ros::Duration(0.0333),
                                          boost::bind(&JacoArmTrajectoryController::update_joint_cartesian,this));
...
void JacoArmTrajectoryController::update_cartesian_force()
{
    {
      boost::recursive_mutex::scoped_lock lock(api_mutex);
      CartesianPosition wrench;
      GetCartesianForce(wrench);
      cartesian_force_[0] = wrench.Coordinates.X;
      cartesian_force_[1] = wrench.Coordinates.Y;;
      cartesian_force_[2] = wrench.Coordinates.Z;;
      cartesian_force_[3] = wrench.Coordinates.ThetaX;
      cartesian_force_[4] = wrench.Coordinates.ThetaY;
      cartesian_force_[5] = wrench.Coordinates.ThetaZ;
    }
     geometry_msgs::WrenchStamped current_wrench;
     current_wrench.header.stamp    = ros::Time::now();
     current_wrench.header.frame_id =  "jaco_link_hand";
     current_wrench.wrench.force.x =  cartesian_force_[0];
     current_wrench.wrench.force.y =  cartesian_force_[1];
     current_wrench.wrench.force.z = cartesian_force_[2];
     current_wrench.wrench.torque.x = cartesian_force_[3];
     current_wrench.wrench.torque.y = cartesian_force_[4];
     current_wrench.wrench.torque.z = cartesian_force_[5];
     cartesian_force_pub_.publish(current_wrench);
}

void JacoArmTrajectoryController::update_joint_cartesian()
{
    {
      boost::recursive_mutex::scoped_lock lock(api_mutex);
      AngularPosition force_data;
      GetAngularForce(force_data);
      joint_eff_c[0] = force_data.Actuators.Actuator1;
      joint_eff_c[1] = force_data.Actuators.Actuator2;
      joint_eff_c[2] = force_data.Actuators.Actuator3;
      joint_eff_c[3] = force_data.Actuators.Actuator4;
      joint_eff_c[4] = force_data.Actuators.Actuator5;
      joint_eff_c[5] = force_data.Actuators.Actuator6;
      joint_eff_c[6] = force_data.Fingers.Finger1;
      joint_eff_c[7] = force_data.Fingers.Finger2;
      joint_eff_c[8] = force_data.Fingers.Finger3;

      AngularPosition velocity_data;
      GetAngularVelocity(velocity_data);
      joint_vel_c[0] = velocity_data.Actuators.Actuator1 * DEG_TO_RAD;
      joint_vel_c[1] = velocity_data.Actuators.Actuator2 * DEG_TO_RAD;
      joint_vel_c[2] = velocity_data.Actuators.Actuator3 * DEG_TO_RAD;
      joint_vel_c[3] = velocity_data.Actuators.Actuator4 * DEG_TO_RAD;
      joint_vel_c[4] = velocity_data.Actuators.Actuator5 * DEG_TO_RAD;
      joint_vel_c[5] = velocity_data.Actuators.Actuator6 * DEG_TO_RAD;
      //NOTE: the finger units are arbitrary, but converting them as if they were in degrees provides an approximately correct visualization
      joint_vel_c[6] = velocity_data.Fingers.Finger1 * DEG_TO_RAD * finger_scale_;
      joint_vel_c[7] = velocity_data.Fingers.Finger2 * DEG_TO_RAD * finger_scale_;
      joint_vel_c[8] = velocity_data.Fingers.Finger3 * DEG_TO_RAD * finger_scale_;

      AngularPosition position_data;
      GetAngularPosition(position_data);
      joint_pos_c[0] = simplify_angle(position_data.Actuators.Actuator1 * DEG_TO_RAD);
      joint_pos_c[1] = position_data.Actuators.Actuator2 * DEG_TO_RAD;
      joint_pos_c[2] = position_data.Actuators.Actuator3 * DEG_TO_RAD;
      joint_pos_c[3] = simplify_angle(position_data.Actuators.Actuator4 * DEG_TO_RAD);
      joint_pos_c[4] = simplify_angle(position_data.Actuators.Actuator5 * DEG_TO_RAD);
      joint_pos_c[5] = simplify_angle(position_data.Actuators.Actuator6 * DEG_TO_RAD);
      joint_pos_c[6] = position_data.Fingers.Finger1 * DEG_TO_RAD * finger_scale_;
      joint_pos_c[7] = position_data.Fingers.Finger2 * DEG_TO_RAD * finger_scale_;
      joint_pos_c[8] = position_data.Fingers.Finger3 * DEG_TO_RAD * finger_scale_;

//      CartesianPosition wrench;
//      GetCartesianForce(wrench);
//      cartesian_force_c[0] = wrench.Coordinates.X;
//      cartesian_force_c[1] = wrench.Coordinates.Y;
//      cartesian_force_c[2] = wrench.Coordinates.Z;
//      cartesian_force_c[3] = wrench.Coordinates.ThetaX;
//      cartesian_force_c[4] = wrench.Coordinates.ThetaY;
//      cartesian_force_c[5] = wrench.Coordinates.ThetaZ;

//      float GravityEstimation[GRAVITY_PARAM_SIZE];
//      GetAngularTorqueGravityEstimation(GravityEstimation);
//            cartesian_force_c[0] = GravityEstimation[0];
//            cartesian_force_c[1] = GravityEstimation[1];
//            cartesian_force_c[2] = GravityEstimation[2];
//            cartesian_force_c[3] = GravityEstimation[3];
//            cartesian_force_c[4] = GravityEstimation[4];
//            cartesian_force_c[5] = GravityEstimation[5];

      AngularPosition angular_wrench;
      //Change gravity vector---------------------------------------------------------------
      // SetGravityType(MANUAL_INPUT);
      // float GravityVector[3];
      // GravityVector[0] = -9.81*sqrt(2)*0.5;
      // GravityVector[1] = 0;
      // GravityVector[2] = 9.81*sqrt(2)*0.5;
      ...
      SetGravityVector(GravityVector);
//      cout << "*********************************" << endl;
//      cout << "*** Gravity = [-9.81*sqrt(2)*0.5, 0, 9.81*sqrt(2)*0.5]" << endl;
      //Change gravity vector end---------------------------------------------------------
      GetAngularForceGravityFree(angular_wrench);
      cartesian_force_c[0] = angular_wrench.Actuators.Actuator1;
      cartesian_force_c[1] = angular_wrench.Actuators.Actuator2;
      cartesian_force_c[2] = angular_wrench.Actuators.Actuator3;
      cartesian_force_c[3] = angular_wrench.Actuators.Actuator4;
      cartesian_force_c[4] = angular_wrench.Actuators.Actuator5;
      cartesian_force_c[5] = angular_wrench.Actuators.Actuator6;

    }
    sensor_msgs::JointState state;
    state.header.stamp = ros::Time::now();
    state.name = joint_names;
    state.position.assign(joint_pos_c.begin(), joint_pos_c.end());
    state.velocity.assign(joint_vel_c.begin(), joint_vel_c.end());
    state.effort.assign(joint_eff_c.begin(), joint_eff_c.end());
    //joint_state_pub_.publish(state);

    geometry_msgs::WrenchStamped current_wrench;
    current_wrench.header.stamp    = state.header.stamp;//ros::Time::now();
    current_wrench.header.frame_id =  "jaco_link_hand";
    current_wrench.wrench.force.x =  cartesian_force_c[0];
    current_wrench.wrench.force.y =  cartesian_force_c[1];
    current_wrench.wrench.force.z = cartesian_force_c[2];
    current_wrench.wrench.torque.x = cartesian_force_c[3];
    current_wrench.wrench.torque.y = cartesian_force_c[4];
    current_wrench.wrench.torque.z = cartesian_force_c[5];
    //cartesian_force_pub_.publish(current_wrench);

    wpi_jaco_msgs::JointPosiCartForce current_jc_state;
    current_jc_state.joint_state = state;
    current_jc_state.cartesian_force = current_wrench;
    joint_cartesian_pub_.publish(current_jc_state);
}
...
bool JacoArmTrajectoryController::loadParameters(const ros::NodeHandle n)
{
    ...
    n.param("wpi_jaco/gripper_closed",          gripper_closed_,       5065.0);
    ...
    n.param("wpi_jaco/serial_number",           serial_number_,         std::string(""));

    n.param<double>("wpi_jaco/finger_stall_interval_seconds", finger_stall_interval_seconds_, 0.5);
    n.param<double>("wpi_jaco/finger_stall_threshold", finger_stall_threshold_, 1.0);
    n.param<double>("wpi_jaco/finger_update_rate_hz", finger_update_rate_hz_, 10.0);
    ...
    ROS_INFO("finger_stall_interval_seconds: %f",    finger_stall_interval_seconds_);
    ROS_INFO("finger_stall_threshold: %f",        finger_stall_threshold_);
    ROS_INFO("finger_update_rate_hz: %f",             finger_update_rate_hz_);
    ...
    cartesian_force_.resize(6);

    joint_pos_c.resize(num_joints_);
    joint_vel_c.resize(num_joints_);
    joint_eff_c.resize(num_joints_);
    cartesian_force_c.resize(6);
...
void JacoArmTrajectoryController::angularTorqueCmdCallback(const wpi_jaco_msgs::AngularTorqueCommand &msg)
{
    if (eStopEnabled)
      return;
    ...
    //Set joint torque command from msg data
    for (int i = 0; i < NUM_JACO_JOINTS; i++)
    {
        TorqueCommand[i]=msg.joints[i];
    }
...
void JacoArmTrajectoryController::cartesianForceCmdCallback(const wpi_jaco_msgs::CartesianForceCommand &msg)
{
    if (eStopEnabled)
      return;
    ...
    //Set joint torque command from msg data
        CartForceCommand[0]=msg.arm.linear.x;
        CartForceCommand[1]=msg.arm.linear.y;
        CartForceCommand[2]=msg.arm.linear.z;
        CartForceCommand[3]=msg.arm.angular.x;
        CartForceCommand[4]=msg.arm.angular.y;
        CartForceCommand[5]=msg.arm.angular.z;


    SendCartesianForceCommand(CartForceCommand);
...
bool JacoArmTrajectoryController::getAngularEffort(wpi_jaco_msgs::GetAngularEffort::Request &req, wpi_jaco_msgs::GetAngularEffort::Response &res)
{
  res.eff.resize(NUM_JACO_JOINTS);
  for (unsigned int i = 0; i < NUM_JACO_JOINTS; i ++)
  {
    res.eff[i] = joint_eff_[i];
  }
...
bool JacoArmTrajectoryController::startForceControlCallback(wpi_jaco_msgs::StartForceControl::Request &req, wpi_jaco_msgs::StartForceControl::Response &res)
{
    if (req.start)
    {
        //Set to position mode first
        //SwitchTrajectoryTorque(POSITION);
        //Set to torque control mode then
        SetTorqueControlType(DIRECTTORQUE);
        // Set the safety factor to 1.0
        ...
        ActuatorDamping[0] = 1.0;
        ActuatorDamping[1] = 1.0;
        ActuatorDamping[2] = 1.0;
        ActuatorDamping[3] = 1.0;
        ActuatorDamping[4] = 1.0;
        ActuatorDamping[5] = 1.0;
        SetTorqueActuatorDamping(ActuatorDamping);
        ...
        // cout << "*********************************" << endl;
        // cout << "*** Gravity = [-9.81*sqrt(2)*0.5, 0, 9.81*sqrt(2)*0.5]" << endl;
        // std::cout<<"Switch to torque control mode. Be careful"<<std::endl;
        SetGravityType(MANUAL_INPUT);
        float GravityVector[3];
        GravityVector[0] = 0;
        GravityVector[1] = 0;
        ...
        SetGravityVector(GravityVector);
        cout << "*********************************" << endl;
        cout << "*** Gravity = [0 0 -9.81]" << endl;
        std::cout<<"Switch to torque control mode. Be careful"<<std::endl;
        //Change gravity vector end---------------------------------------------------------
        SwitchTrajectoryTorque(TORQUE);
        ros::Duration(1.0).sleep();
        int TrajectoryTorqueMode = 0;
        // Get the angular torque gravity estimation
        GetTrajectoryTorqueMode(TrajectoryTorqueMode);//This function returns the mode. 0: Trajectory mode 1: Torque mode.
        if (TrajectoryTorqueMode)
        {
            ROS_INFO_STREAM("RESULT OF TORQUE CONTROL START: " << "SUCCESS");
        }
        else
        {
            ROS_INFO_STREAM("RESULT OF TORQUE CONTROL START: " << "FAIL");
        }
        res.success = TrajectoryTorqueMode;
    }

    return true;
}

bool JacoArmTrajectoryController::stopForceControlCallback(wpi_jaco_msgs::StopForceControl::Request &req, wpi_jaco_msgs::StopForceControl::Response &res)
{
    if (req.stop)
    {
        int a = 0;
        a = SwitchTrajectoryTorque(POSITION);
        ROS_INFO_STREAM("RESULT OF TORQUE CONTROL STOP:" << a);
        res.success = a;
    }
