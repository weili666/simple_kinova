...
#include <wpi_jaco_msgs/FingerPosition.h>
...
{
public:
    FingerAngles():Finger1(0.0), Finger2(0.0),Finger3(0.0){}
    FingerAngles(double f1, double f2, double f3):Finger1(f1), Finger2(f2),Finger3(f3){}
    FingerAngles(const wpi_jaco_msgs::FingerPosition& fp):Finger1(fp.finger1), Finger2(fp.finger2), Finger3(fp.finger3){}
    bool isCloseToOther(FingerAngles& other, double tolerance)
    {
        bool status = true;
    ...
    wpi_jaco_msgs::FingerPosition constructFingersMsg()
    {
        wpi_jaco_msgs::FingerPosition angles;
...
public:
  ...
  typedef actionlib::SimpleActionServer<wpi_jaco_msgs::SetFingersPositionAction>	FingersServer;
  ...
  /**
   * \brief Reads cartesian force from the arm and publishes them as wrench stamped message
   */
  void update_cartesian_force();

  /**
   * \brief Reads joint states and cartesian force from the arm and publishes them as user-defined message
   */
  void update_joint_cartesian();
  ...
  /**
   * @brief execute_fingers Callback for the fingers_server_
   */
  void execute_fingers(const wpi_jaco_msgs::SetFingersPositionGoalConstPtr &);
...
private:
  ...
  /**
   * \brief Callback for sending an angular toruqe command to the arm
   * @param msg angular torque command and info
   */
  void angularTorqueCmdCallback(const wpi_jaco_msgs::AngularTorqueCommand& msg);

  /**
   * \brief Callback for sending a cartesian force command to the arm
   * @param msg cartesian force command and info
   */
  void cartesianForceCmdCallback(const wpi_jaco_msgs::CartesianForceCommand& msg);
  ...
  */
  ...
  /**
   * \brief Service callback for getting the current joint efforts of the arm
   *
   * This allows other nodes to request the joint efforts when needed, rather than listening to the joint_state
   * topic.
   * @param req empty service request
   * @param res service response including joint efforts
   * @return true on success
   */
  bool getAngularEffort(wpi_jaco_msgs::GetAngularEffort::Request &req, wpi_jaco_msgs::GetAngularEffort::Response &res);
  ...
  */
  ...
  /**
  * \brief Callback for start force control
  *
  * @param req start force control service request
  * @param res success or not service response
  */
  bool startForceControlCallback(wpi_jaco_msgs::StartForceControl::Request &req, wpi_jaco_msgs::StartForceControl::Response &res);

  /**
  * \brief Callback for start force control
  *
  * @param req stop force control service request
  * @param res success or not service response
  */
  bool stopForceControlCallback(wpi_jaco_msgs::StopForceControl::Request &req, wpi_jaco_msgs::StopForceControl::Response &res);
  ...
  ros::Subscriber angularTorqueCmdSubscriber; //!< subscriber for angular arm torque command
  ros::Subscriber cartesianForceCmdSubscriber; //!< subscriber for cartesian arm force command
  ros::Publisher cartesian_force_pub_;//!< publisher for cartesian force
  ros::Publisher joint_cartesian_pub_;//!< publisher for joint states and cartesian force
  ...
  ros::ServiceServer angularEffortServer; //!< service server to get the joint efforts
  ...
  ros::ServiceServer startForceControlServer; //!< start the force control mode
  ros::ServiceServer stopForceControlServer; //!< stop the force control mode
  ros::ServiceServer eraseTrajectoriesServer;

  ros::Timer joint_state_timer_; //!< timer for joint state publisher
  ros::Timer cartesian_force_timer_;//!< timer for cartesian force publisher
  ros::Timer joint_cartesian_timer_;//!< timer for joint states and cartesian force publisher
  ...
  ros::Time finger_last_nonstall_time_;
  jaco::FingerAngles finger_last_nonstall_positions_;
  double finger_stall_interval_seconds_;
  double finger_stall_threshold_;
  double finger_update_rate_hz_;
  ...
  std::vector<double>      cartesian_force_;

  std::vector<double>      joint_pos_c;
  std::vector<double>      joint_vel_c;
  std::vector<double>      joint_eff_c;
  std::vector<double>      cartesian_force_c;
